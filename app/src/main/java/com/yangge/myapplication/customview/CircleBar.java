...
public class CircleBar extends View {
    ...
    private int mTextColor = getResources().getColor(R.color.blue);//默认数字颜色
    private int mDesTextColor = getResources().getColor(R.color.gray2);//默认描述文字颜色
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        mTextDesSize = dip2px(getContext(), 18);
        ...
        textDesPaint.setColor(mDesTextColor);
...
//        canvas.drawArc(mColorWheelRectangle, -225, 243, false, mColorWheelPaint);//画圆环
...
//                , textDesPaint);
        canvas.drawText(mTextDes, mColorWheelRectangle.centerX() - bounds.width()/2,
                (float) (mColorWheelRectangle.centerY() + mColorWheelRectangle.centerY()/Math.sqrt(2))
                , textDesPaint);
    }
...
//                * 然后调用postInvalidate()不停的绘制view。
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            if (interpolatedTime < 1.0f) {
                ...
                mCount = new DecimalFormat(".00").format(interpolatedTime * Float.parseFloat(mText));
            } else {
